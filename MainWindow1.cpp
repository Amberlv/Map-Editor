...
QPainter painter(this); // 获取窗口绘制工具
QPen pen(Qt::black, 3);
QPainterPath path;

//scene->clear(); // 清空场景中所有的元素
if (!fileName.isNull())
{
    loadMap(fileName);
}
...
for (int i = 0; i < P.size(); i++)
{
    ...
    QGraphicsEllipseItem* ellipseItem = scene->addEllipse(P.at(i).x() - 2, P.at(i).y() - 2, 4, 4, pen);
}
for (int i = 0; i < L.size(); i++)
{
    if (L[i].size() != 2)
    ...
    if (pt == &line && L.at(i).size() == 2)
    {
        ...
        line.draw_shape(L[i][0], L[i][1], path);
        // 添加线到场景中
    ...
    for (int i = 0; i < C.size(); i++)
    {
        ...
        if (C.at(i).size() == 2)
        {
            ...
            circle.draw_shape(C[i][0], C[i][1], path);
            scene->addPath(path, pen);
    ...
    for (int i = 0; i < T.size(); i++)
    {
        ...
        if (T.at(i).size() == 3)
        {
            triangle.draw_shape(T[i][0], T[i][1], T[i][2], path);
            scene->addPath(path, pen);
    ...
    for (int i = 0; i < R.size(); i++)
    {
        ...
        if (R[i].size() == 4)
        {
            rectangle.draw_shape(R[i][0], R[i][1], R[i][2], R[i][3], path);
            scene->addPath(path, pen);
    ...
    for (int i = 0; i < col_pl.size(); i++)
    {
        ...
        case 1:
        {
            ...
            polygon.fill(PL[col_pl[i][0]], path);
            scene->addPath(path, pen);
            break;
        }
        case 2:
    ...
    for (int i = 0; i < col_r.size(); i++)
    {
        ...
        {
            pen = (QPen(Qt::red, 3));
            rectangle.fill(R[col_r[i][0]], path);
            scene->addPath(path, pen);
            break;
        }
        case 2:
    ...
    if (is_select && choose_count == 1 && I.size() != 0)//选中图形高亮显示
    {
        ...
        if (I[c][0] == 1)
        {
            line.draw_shape(L[I[c][1]][0], L[I[c][1]][1], path);
            scene->addPath(path, pen);
          
        }
        else if (I[c][0] == 2)
        {
            circle.draw_shape(C[I[c][1]][0], C[I[c][1]][1], path);
            scene->addPath(path, pen);
           
        }
        else if (I[c][0] == 3)
        {
            triangle.draw_shape(T[I[c][1]][0], T[I[c][1]][1], T[I[c][1]][2], path);
            scene->addPath(path, pen);
            
        }
        else if (I[c][0] == 4)
        {
            rectangle.draw_shape(R[I[c][1]][0], R[I[c][1]][1], R[I[c][1]][2], R[I[c][1]][3], path);
            scene->addPath(path, pen);
           
        }
        else if (I[c][0] == 5)
        {
            polygon.draw_shape(PL[I[c][1]], path);
            scene->addPath(path, pen);
